...
function scope (result) {
  ...
  const initial = {}
  collectionScope(result, initial)
  const args = Object.keys(initial).length ? JSON.stringify(initial) : ''
  lines.push(`postman[Scope](${args});`)
  return lines.join(`\n`)
}

function collectionScope (result, initial) {
  if (result.scope.collection) {
    initial.collection = result.scope.collection
  }
}
