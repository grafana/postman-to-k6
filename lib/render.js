...
function dataParse (result) {
  switch (result.data.type) {
    case 'csv':
      throw new Error('CSV data files not yet implemented')
    case 'json':
      return `const rows = JSON.parse(text);`
    default:
...
function iteration (spec) {
  if (spec.data && spec.data.path) {
    return `postman[Iteration](); // Advance data variables`
  } else {
    return null
  }
}
