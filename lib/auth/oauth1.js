...
${util.indent(request.join(`,\n`))}
    ...
    if (token.length) {
      ...
      this.setup.push(`token = {
${util.indent(token.join(`,\n`))}
}`)
    }

    // Sign
    this.setup.push(`oauth = OAuth(options)`)
    const args = []
    args.push(`request`)
    if (token.length) {
      args.push(`token`)
    ...
    switch (location) {
      ...
      case Location.Address:
        this.setup.push(`auth = oauth.authorize(${args.join(`, `)})`)
        break
    }
    ...
    switch (location) {
      ...
      case Location.Address:
        ...
        this.setup.push(`address = new URI(${address});`)
        this.setup.push(`for (const key of Object.keys(auth)) {
