...
function requireScope () {
  if (!state.scope) {
    throw new Error('May only be used in local scope')
  }
}
...
const postman = Object.freeze({
  ...
  /**
   * Execute Postman scope
   *
   * Executes a Postman request and surrounding logic with a scoped set of
   * local variables. Exposes test script specific pm.* surface for the post
   * request logic. Executes provided logic synchronously.
   *
   * @param {function} body - Prerequest and request logic.
   * @param {function} [post] - Test script logic.
   */
  [Scope] (body, post) {
    try {
      // Body
