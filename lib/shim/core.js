...
const postman = Object.freeze({
   ...
   * variables. Exposes test script specific pm.* surface for the post request
   * logic. Executes provided logic synchronously.
   *
   * @param {Array} args - Request arguments. Passed to `http.request`.
   * @param {function} [pre] - Prerequest logic.
   * @param {function} [post] - Postrequest logic. Receives k6 response.
   * @return k6 response.
   */
  [Request] (args, pre, post) {
    try {
      state.request = true
      // TODO: Expose request scoped properties.
      if (pre) {
        pre()
      }
      // TODO: Send request.
      const response = undef
      if (post) {
        state.post = true
        // TODO: Expose test scoped properties.
        exposePm()
        post(response)
      }
      return response
      ...
      state.request = false
      state.post = false
      state.local = new Dict()
      // TODO: Scrub scoped properties
      exposePm()
    }
  }
})
