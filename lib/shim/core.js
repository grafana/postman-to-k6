...
const postman = Object.freeze({
  ...
  [Request] ({
    ...
    try {
      ...
      executePrerequest(this[Pre], pre)
      ...
      if (post) {
        enterPost(response)
        executePostrequest(this[Post], post, response)
        executeTests()
      }
      return response
    } finally {
  ...
  [Define] (spec) {
    ...
    definition.request[name].push(spec)
  },

  /* Execute named request */
  [NamedRequest] (name, number = 1) {
    if (!definition.request[name]) {
      throw new Error(`No request defined with name '${name}'`)
    }
    if (!(definition.request[name].length >= number)) {
      throw new Error(
        `No request defined with name '${name}' and number ${number}`
      )
    }
    const spec = definition.request[name][number - 1]
    return postman[Request](spec)
  },
  ...
  [Extend]: {
    ...
    jsonSchema () {
      throw new Error(
        'To use JSON schema import "./libs/shim/jsonSchema.js"'
      )
    },
    jsonSchemaNot () {
      throw new Error(
        'To use JSON schema impot "./libs/shim/jsonSchema.js"'
      )
    },
    module: {}
  }
    ...
    header (key, value) {
      if (arguments.length > 1) {
        store.test.push(response =>
          response.headers.uncased[key.toLowerCase()] === value
        )
      } else {
        store.test.push(response =>
          key.toLowerCase() in response.headers.uncased
        )
      }
    },
    jsonBody () {
