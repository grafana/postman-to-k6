...
function Request (request, result, block) {
  ...
  if (request.auth) {
    authentication(request.auth, request, headerHash, url, post)
  }
...
function authentication (settings, request, headerHash, url, post) {
  const { type } = settings
  if (util.validAuth(type)) {
    const auth = Auth[type]
    if (!auth.enabled) return
    const header = auth.header(request)
    if (header) headerHash[header.key] = header.value
  } else post.push(`/* Note: ${type} authentication not supported */`)
}
