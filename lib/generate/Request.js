...
function authentication (settings, feature, result, block) {
  ...
  if (util.validAuth(settings.type)) {
    authValid(settings, feature, result, block)
  } else authInvalid(settings, feature)
}
function authValid (settings, feature, result, block) {
  const auth = new Auth[settings.type](settings, feature)
  if (auth.imports) authImports(auth, result)
  if (auth.declares) authDeclares(auth, block)
  if (auth.headers) authHeaders(auth, feature)
  if (auth.headerSpread) authHeaderSpread(auth, feature)
  if (auth.options) authOptions(auth, feature)
  if (auth.credential) authCredential(auth, feature)
  if (auth.address) authAddress(auth, feature)
  if (auth.pre) authPre(auth, feature)
}
function authImports (auth, result) {
  for (const [ name, spec ] of auth.imports) result.imports.set(name, spec)
}
function authDeclares (auth, block) {
  for (const declare of auth.declares) block.declares.add(declare)
}
...
function authHeaderSpread (auth, feature) {
  for (const item of auth.headerSpread) feature.headerSpread.add(item)
}
...
function authAddress (auth, feature) { feature.address = auth.address }
function authPre (auth, feature) { feature.pre.push(...auth.pre) }
...
function render (feature, result, block) {
  ...
  if (typeof feature.address === 'string') args.push(feature.address)
  else args.push(util.evalString(feature.address.toString()))
  if (feature.data) args.push(`"${feature.data}"`)
  else if (feature.method !== 'get') args.push(`null`)
...
function renderHeaders (headers, spread, options) {
  if (!(headers.size || spread.size)) return
...
function renderHeaderSpread (target, spread) {
  if (!spread.length) return target
  const source = spread.pop()
  if (!target) return renderHeaderSpread(source, spread)
