...
function render (feature, result, block) {
  ...
  const config = {
    method: JSON.stringify(feature.method),
    address: renderAddress(feature.address),
    data: renderData(feature.data, feature.dataSpread),
    options: renderOptions(feature.options)
  }
  const args = renderArgs(config, feature.pre, feature.post)
...
function renderArgs (config, pre, post) {
  const args = []
  args.push(renderConfig(config))
  if (pre.length || post.length) {
    args.push(renderPre(pre))
  }
  if (post.length) {
    args.push(renderPost(post))
  }
  return args
}

function renderConfig (config) {
  const list = listConfig(config)
  return `[${list.join(`, `)}]`
}
...
function listConfig (config) {
  ...
  list.push(config.address)
  if (config.data || config.options) {
    list.push(config.data || `null`)
  }
  if (config.options) {
    list.push(config.options)
  }
  return list
}
...
function renderAddress (address) {
  if (typeof address === 'string') {
    ...
    return util.evalString(address.toString())
  }
}
