...
function render (feature, result, block) {
  ...
  const shortcut = METHOD.has(method)
  const call = `http.${shortcut ? method : `request`}`
  const args = []
  if (!shortcut) args.push(JSON.stringify(method))
  args.push(`"${url}"`)
  if (feature.data) args.push(`"${feature.data}"`)
  else if (feature.headers.size && method !== 'get') args.push(`null`)
