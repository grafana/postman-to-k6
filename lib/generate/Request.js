...
function Request (name, request, result, block) {
  ...
  if (result.setting.separate) {
    renderSeparate(name, id, feature, result, block)
  } else {
    renderInline(name, id, feature, result, block)
  }
}
...
function renderSeparate (name, id, feature, result, block) {
  const config = renderConfig(name, id, feature)
  stageSeparate(name, config, result)
  renderSeparateCall(name, result, block)
}

function stageSeparate (name, config, result) {
  const request = result.request
  if (!request[name]) {
    request[name] = []
  }
  request[name].push(config)
}

function renderSeparateCall (name, result, block) {
  const request = result.request
  const number = request[name].length
  const args = []
  args.push(JSON.stringify(name))
  if (number > 1) {
    args.push(number.toString())
  }
  block.main.push(`postman[NamedRequest](${args.join(`, `)});`)
}

function renderInline (name, id, feature, result, block) {
  const config = renderConfig(name, id, feature)
