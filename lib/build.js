...
async function buildCompat (polyfills, bundles, imports) {
  if (!(bundles.size || imports.size)) {
    return null
  }
  const index = renderIndex(polyfills, bundles, imports)
...
function renderIndex (polyfills, bundles, imports) {
  return [
    renderEffects(polyfills),
    renderExports(bundles, imports)
  ].join(`\n\n`)
}

function renderEffects (polyfills) {
  const lines = []
  for (const name of polyfills) {
    lines.push(`require(${JSON.stringify(name)});`)
  }
  return lines.join(`\n`)
}

function renderExports (bundles, imports) {
  const entries = []
